...
def cifrar_mensaje(mensaje, public_key_base64):
    try:
        ...
        cipher = Cipher(
            algorithms.AES(key_simetrica), modes.CBC(iv), backend=default_backend()
        )
        ...
        return {
            ...
            "clave_simetrica": base64.b64encode(clave_simetrica_cifrada).decode(
                "utf-8"
            ),
...
def descifrar_mensaje(datos_cifrados, clave_simetrica_cifrada, iv):
    try:
        ...
        mensaje_con_relleno = (
            decryptor.update(base64.b64decode(datos_cifrados)) + decryptor.finalize()
        )
